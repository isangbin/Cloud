...
- 유저 정보
 ...
 - 유저 생성
    ...
    - label을 없애고 widget을 통해 placeholder를 추가해 디자인적으로 깔끔하도록 하였다.
 - 유저 수정
...
- select
  - 해당 페이지는 유저가 회원 가입 후 바로 연결되거나 로그인 상태로 select페이지로 이동할 수 있다.
  - database에 담긴 영화 중 유저가 좋아하는 영화를 저장해 둘 수 있다.
  - 선택된 영화는 흐리게 표시되어 시각적으로 구분할 수 있다.
  - 선택된 영화들은 개인 프로필 페이지에서 확인할 수 있으며 추천 영화에도 반영된다.
...
- detail
  - detail 페이지는 database가 아니라 실시간으로 가져온 영화에 대한 정보를 표시해야할 수 도 있기 때문에 유저가 특정 영화를 클릭 시 detail 페이지에서 다시 axios를 활용해 영화 정보를 불러왔다.
...
- community
  - model
     - Review 모델은 다음의 field를 가진다.
        - title : 게시글의 제목
        - movie_title : 게시글을 작성하려는 관련 영화 제목
        - rank : 영화의 평점을 저장
        - content : 게시글 내용
        - created_at : 게시글 생성 시점
        - updated_at : 게시글 수정 시점
        - user : 해당 게시글을 작성한 user 정보, ForeignKey로 N : 1관게 형성
        - like_users : 해당 게시글에 좋아요를 누른 유저 정보, ManyToManyField로 유저와 M : N 관계

     - Comment 모델은 다음의 field를 가진다.
        - review : 댓글은 단 영화와 ForeignKey를 통해 N : 1 관게 형성
        - user : 댓글을 작성한 유저와 ForeignKey를 통해 N : 1 관계 형성
        - content : 댓글 내용
        - like_users : 댓글에 좋아요를 누른 사람과 ManyToManyField로 M : N 관계 형성
  
  - index
     - index페이지에서는 현재 작성되어 있는 게시글들을 확인할 수 있다.
     - 게시글들은 최신 순으로 최상단에 저장된다.
     - 게시글 마다 좋아요를 누를 수 있고 좋아요를 누른 개수는 개인 프로필 페이지에 표시된다.
     - idnex에서는 게시글들의 제목을 위주로 표시되고 각 게시글 마다 자세한 내용을 볼 수 있는 detail페이지로 이동하는 버튼이 존재한다.
  - create
  ...
  - detail
     - index 페이지에서 detail 버튼 클릭시 이동할 수 있다.
     - 게시글에 대한 자세한 정보를 확인할 수 있고 그 항목은 상기의 Review 모델의 항목과 같다.
     - 댓글을 작성할 수 있고 각각의 댓글들 또한 좋아요를 누를 수 있다.
